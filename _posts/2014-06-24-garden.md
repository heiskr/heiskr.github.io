...
- Event-driven.
- Implied type with minimal implicit conversion.
...
**Modules.** Each file is considered a unique namespace. To import data and functions from other files, use the `import` function. `import` is the only explicitly globally defined function. (Function aliases, as we'll see a bit later on, are also globally available.) If a cycle is formed with `import`, the compiler will throw an error.

    import('./path/to/module')

By default, everything in the module is made available. You can specify with the second argument a more limited set.
...
Some modules are part of the standard library, such as the `type` module.
    ...
    import('type')
    a = toString(1)  ; converts type
    b = getType(2)  ; returns type as a string

A functional alias of import is `@`. When using the alias, strings are assumed.

    @ ./path/to/module
    @ ./path/to/module, a, b, c

Data Types
----------

Data types in Garden are purely data, and do not have any operations (methods, functions) of their own. In the below examples, we will use function aliases, as described in a later section, for syntactical convenience. Every data type has one, and only one, "falsy" representation. Most data types are immutable by default; they are essentially cloned when making updating operations. The mutable versions of data types are typically prefixed by `$`.
...
**Number.** There is only one type of number. Numbers have by far the largest number of function aliases available. `0` is the falsy value. Garden will try to keep representations for as long as possible, ensuring that the final output has the best floating point representation possible. Numbers are always immutable.
...
**Tuple.** Tuples are defined with `[]`. Tuples can be multi-dimensional (embedded). Tuples are zero-indexed. The falsy value of tuple is the empty tuple, `[]`.
...
**Array.** Arrays are defined with `$[]`. Arrays are almost completely identical to tuples. The only exception is that arrays are mutable, and array operations will change the contents of the array. Arrays are dynamically sized.
...
When both array and tuple libraries are imported, the code will determine the appropriate method to use.

**Vectors.** Vectors are immutable. Vectors are defined as `vector[]` or `vector{}`, supporting implied numeral keys (`[]`) or other types of keys `{}`. The falsy vector is an empty vector.

**Matrixes.** Matrixes are immutable. Matrixes are defined either as `matrix[]` or `matrix{}`, supporting implied numeral keys (`[]`) or other types of keys `{}`. The falsy matrix is an empty matrix.

**Hash.** Hases are defined with `{}`. Hashes are immutable. Hashes are unordered. Hashes support embedding. The falsy form of hash is an empty hash, `{}`.
    ...
    q.a  ; Get a value
    delete(q, 'a')  ; Creates a new hash

**Object.** Objects are defined with `${}`. Objects are almost completely identical to hashes. The only exception is that objects are mutable, and object operations will change the contents of the object.
...
When both hash and object libraries are imported, the code will determine the appropriate method to use.
...
Functions are always in keyword style. This means argument names can always be added for clarity. Without argument names, functions assume the ordering matches the arguments.
...
Splats can be used to refer to an addition arguments. Splats are identified as `...`, and may use numeric indexes or strings.
...
**Conditionals.** `if` statements do not convert type. If you try to compare two different types, you will get an error.
...
Thanks to the functional aliases, chained comparisons are allowed.

    if a > b > c
        return true

A shorthand is also possible for assignment.

    c = if a is b then true else false
...
While being largely a functional language (with a few mutable data types), aliases give Garden a much more familiar syntax.

Unlike their functional equivalents, aliases do observe standard order of operations.
...
- `and` and `&&` are aliases of `and()`
- `or` and `||` are aliases of `or()`
- `not`, `~`, and `!` are aliases of `not()`
- `is`, `eq`, and `==` are aliases of `equals()`
- `isnt` and `!=` are aliases of `notEquals()`
- `lt` and `<` are aliases of `lessThan()`
- `gt` and `>` are aliases of `greaterThan()`
- `lte` and `<=` are aliases of `lessThanOrEquals()`
- `gte` and `>=` are aliases of `greaterThanOrEquals()`
- `.` aliases `get()` and `set()`. `.` interpolate type as number or string
...
Several control statements are not functional aliases. `if`, `else`, `for`, `while`, `break`, `continue`, `return`, `try`, `catch`, `assert`, and `throw` are not aliases.

Custom Data Types
-----------------

When creating custom data types, other aliases can be defined. Custom data types can be defined. TODO
...
- Type must be explicitly converted for comparison.
...
- Mutable variable name should be prefixed with `$`.
