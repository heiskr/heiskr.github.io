...
**Data**. Data by default is immutable, though mutable types are available. Data does not own functions. Examples are boolean, numbers, strings, maps, and tuples.
...
Tuples can only store other immutable data types, such as boolean, number, string, tuple, and hash.
...
The `get` and `set` methods exist on all tuples, lists, hashes, and objects, respecting the mutablility characteristic. A `set` operation will always return the full value of the iterable.

    set a to (get 'key' in my_hash)
...
Prepositions are using to keyword all arguments after the first argument.
...
The anonymous function is defined as: `-> (given) arg1 (preposition) arg2 ... \n block`
...
With loops are aware of the data type.
    ...
    set my_hash to {'a':1 'b':2 'c':3}
    
    with num in my_tuple
    with [index, num] of my_tuple
    with value in my_hash
    with [key, value] of my_hash
...
*Alias: Getters and Setters.* Many languages allow using `object.key` and `object[key]` for getters and seters of iterables, and Garden's alias can allow for that as well.

    set a to my_hash.key
    set b to my_tuple[0]
...
*Alias: Inline-block.* Sometimes, having to hit return just for a single-line block doesn't feel right. This alias enables a work-around. The colon character here replaces the newline plus indent.
    ...
    map lis by (-> value: divide value by 3)
...
*Alias: Type Annotation.* With this alias enabled, function arguments and returns can have a type specified. Type annotations are checked at build-type and not run-time.
    ...
    set my_add to -> num1:number to num2:number
